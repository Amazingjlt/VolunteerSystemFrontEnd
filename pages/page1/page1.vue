		...
		<view class="score-section">
			...
			<form class="score">
				<view class="input-group">
					<text>语文：</text>
					<input v-model="formData.chinese" type="number" placeholder="满分[120]分" min="0" max="120"
						@blur="validateField('chinese', 120)" />
				</view>
				<view class="input-group">
					<text>数学：</text>
					<input v-model="formData.math" type="number" placeholder="满分[120]分" min="0" max="120"
						@blur="validateField('math', 120)" />
				</view>
				<view class="input-group">
					<text>英语（含听说）：</text>
					<input v-model="formData.english" type="number" placeholder="满分[120]分" min="0" max="120"
						@blur="validateField('english', 120)" />
				</view>
				<view class="input-group">
					<text>物理：</text>
					<input v-model="formData.physics" type="number" placeholder="满分[100]分" min="0" max="100"
						@blur="validateField('physics', 100)" />
				</view>
				<view class="input-group">
					<text>化学：</text>
					<input v-model="formData.chemistry" type="number" placeholder="满分[100]分" min="0" max="100"
						@blur="validateField('chemistry', 100)" />
				</view>
				<view class="input-group">
					<text>历史：</text>
					<input v-model="formData.history" type="number" placeholder="满分[60]分" min="0" max="60"
						@blur="validateField('history', 60)" />
				</view>
				<view class="input-group">
					<text>政治：</text>
					<input v-model="formData.politics" type="number" placeholder="满分[60]分" min="0" max="60"
						@blur="validateField('politics', 60)" />
				</view>
				<view class="input-group">
					<text>体育：</text>
					<input v-model="formData.physical" type="number" placeholder="满分[60]分" min="0" max="60"
						@blur="validateField('physical', 60)" />
				</view>
			</form>
		</view>
...
<style scoped>
	.container {
		...
		/* Increased padding to ensure content doesn't overlap with fixed button */
	}
	...
	.spacer {
		height: 120rpx;
		/* Spacer to ensure content doesn't overlap with fixed footer */
	}
	...
	.footer {
		...
		background: #fff;
		/* White background to prevent transparency */
		z-index: 1000;
